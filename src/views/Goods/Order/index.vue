...
import { onGetOrderList, onShowProgressDialog } from '@/api/order'
export default {
  data () {
    return {
      // 订单列表查询参数
  ...
  methods: {
    ...
    getOrderList () {
      ...
      onGetOrderList(params).then(res => {
        if (res.data.meta.status !== 200) {
          return this.$message.error('获取订单列表失败')
        }
        this.orderList = res.data.data.goods
        this.total = res.data.data.total
        console.log('返回订单数据列表res', res)
      })
    },
    ...
    showProgressDialog () {
      ...
      onShowProgressDialog(params).then(res => {
        if (res.data.meta.status !== 200) {
          return this.$message.error('获取物流进度失败!')
        }
        this.progressInfo = res.data.data
        this.progressDialogVisible = true
        console.log('物流进度数据', this.progressInfo)
      })
    }
  }
}
